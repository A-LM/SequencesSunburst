...
d3.csv("https://raw.githubusercontent.com/A-LM/SequencesSunburst/main/Radial%20Data.csv", function(error, data) {
...
const adaptedRoot = 'Adapted Root';
const adaptedHierarchy = 'Adapted Hierarchy';
_1(adaptedRoot, adaptedHierarchy);
...
async function _csv(d3, FileAttachment) {
  return d3.csvParseRows(
    await FileAttachment(
      "https://raw.githubusercontent.com/A-LM/SequencesSunburst/main/Radial%20Data.csv"
    ).text()
  );
}
...
function _color(d3, data) {
  const uniqueValues = Array.from(new Set(data.map(d => d["SCOP PROIECT"])));
  return d3
    .scaleOrdinal()
    .domain(uniqueValues)
...
const data = await _csv(d3,FileAttachment("https://raw.githubusercontent.com/A-LM/SequencesSunburst/main/Radial%20Data.csv"));
const color = _color(d3, data);
